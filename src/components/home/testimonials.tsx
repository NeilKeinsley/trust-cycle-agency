...
export function Testimonials() {
  ...
  return (
    <Panel tone="light" padClassName="py-10 lg:py-12">
      ...
      <div className="grid gap-6 lg:grid-cols-3 lg:grid-rows-2 lg:gap-4">
        {feature && (
          <Reveal delay={140} className="lg:col-span-2 lg:row-span-2">
            <figure className="flex h-full min-h-[22rem] flex-col justify-between rounded-[var(--radius-card)] border border-line bg-card p-8 lg:p-10">
              <span aria-hidden="true" className="font-mono text-5xl leading-none text-accent/30 lg:text-6xl">
                &ldquo;
              </span>
              <blockquote className="mt-4 flex-1 text-2xl leading-snug text-foreground/90 sm:text-3xl lg:text-[1.875rem]">
                {feature.quote}
              </blockquote>
        ...
        {rest.map((t, i) => (
          <Reveal key={t.name} delay={220 + i * 100}>
            <figure className="flex h-full min-h-[10rem] flex-col rounded-[var(--radius-card)] border border-line bg-card p-7 lg:p-6">
