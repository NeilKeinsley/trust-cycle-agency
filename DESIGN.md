...
A full-bleed Studio Night hero that fills the viewport. At `lg` it splits: headline, lead and the pill selector card (a translucent blurred container) in 7 columns, the Studio Stack image in 5; on phones the image follows the selector, and on mid-size tablets it is omitted because the pinned hero has a fixed height. Trust notes with thin line icons run beneath. Its bottom edge is a single-row client rail divided by Night Grey hairlines: each cell is a small geometric client mark in Trust Teal, the client name and a Label caption. The rail scrolls on a 40s linear loop, pauses on hover and focus, fades at both edges, and becomes a plain swipeable strip under reduced motion.

### Studio Stack (hero image)
The hero's picture is the work itself, drawn in tokens: a brand sheet, a campaign chart, a live website, a search result and a fresh lead card (Trust Teal "Qualified" badge) resting as a loose pile over the cycle ring (the old concentric arcs, one ring in Trust Teal). Cards are Studio Night with Night Grey hairlines and an inset top highlight, never a dark drop shadow. Motion is the hero's one authored moment: on load the cards settle into the pile (staggered, spring-eased); as the hero scrolls away, a scroll-driven timeline pulls the pile apart into an exploded view while the ring turns, and on desktop the pile lingers against the scroll so the moment plays in view until the next panel covers it. The client rail sits on its own layer above the lingering cards. Reduced motion or no scroll-timeline support: the pile simply rests.
